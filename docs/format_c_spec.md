...
**사용 예시 ([도메인 라벨]):**
1. (사용자) "..." — 자연어 프롬프트
2. (AI 응답을 보고 사용자) "..." — 다음 프롬프트
3. ...
```

- 각 단계는 **자연어 메시지** (따옴표로 감싸기 권장)
- 사용자가 친 메시지가 짧으면 짧은 채로 ("응 진행해줘", "그렇게 해줘" 도 OK — 실제로 자주 쓰는 패턴)
- 도구 이름·MCP 이름·명령어가 사용자 발화 안에 들어있을 수는 있음 (사용자가 직접 "grafana 에서 봐줘" 라고 친 경우)
- 단계 사이에 짧은 설명을 괄호로 (선택): `1. "..." (관련 로그 한꺼번에 떨어짐)`
- **3-6 단계**
...
1. "운영에서 5xx 떨어지는 거 같은데 시간대 14시쯤 로그 좀 봐줘"
2. (AI 가 로그 정리해서 보여주면) "응답 느린 user_id 들 추출해줘"
3. "같은 trace_id 로 인증 서비스 로그도 이어서 봐줘"
4. "이제 코드에서 어디가 원인인지 찾자"
```

✅ 좋음 (짧은 승인 포함):
```
**사용 예시 (Git / 회귀 추적):**
1. "이거 언제부터 이렇게 됐지? 최근 커밋들 좀 봐줘"
2. "의심되는 커밋 직전 시점 동작 재현해줘"
3. "그럼 그 사이 diff 만 좁혀서 분석해줘"
4. "이 부분이 원인 같네. 고쳐줘"
5. "응 진행해줘"
```
