...
**(4) outcome 시퀀스** — 에피소드 안에서 outcome 신호가 발생한 **순서**:

단일 outcome set 만으로는 "데이터 → 실행 → 데이터" 처럼 둘러싸는 검증 패턴을 못 잡는다. 시퀀스를 별도로 보존.

```
outcome_sequence: list[str]   # 시간순. 예: ["verified_by_data", "verified_by_run", "verified_by_data", "committed"]
```

생성 규칙 (결정론):
- 에피소드 turn 들을 시간순으로 훑으며 turn 의 `outcome_signals` 가 비어있지 않은 첫 등장만 시퀀스에 추가
- 동일 outcome 이 연속하면 한 번만 (`["committed", "committed"]` → `["committed"]`)
- phase 무관 — verify phase 가 아닌 곳에서 발생한 신호도 시퀀스엔 포함 (시퀀스의 의미는 "어떤 순서로 어떤 검증·종결을 거쳤나" 이므로)
- 단, episode-level `outcomes` set 은 기존 규칙 유지 (phase 필터 적용)

이 시퀀스를 보고 Stage D 가 시퀀스 패턴 (예: `verified_by_data → verified_by_run → verified_by_data`) 을 별도 신호로 다룸.

### 에피소드 합산 결정론 룰

turn 단위 신호가 있으면 episode 단위 `outcomes` 가 반드시 채워져야 한다. **LLM 합산에 의존하지 말 것.**

```python
# 결정론 합산 (Stage B 가 반드시 수행)
episode_outcomes = set()
for turn in episode.turns:
    for tu in turn.tool_uses:
        for sig in (tu.outcome_signals or []):
            base = sig.replace("_candidate", "")
            # phase 필터
            if base in ("verified_by_data", "verified_by_run") and turn.phase != "verify":
                continue
            if base in ("committed", "pushed", "pr_opened", "pr_with_structured_body") and turn.phase != "verify":
                # 도입부에서 git status 확인용 commit 표시 같은 false-positive 방지
                continue
            episode_outcomes.add(base)
```

`_candidate` 접미사가 붙은 turn 신호 (`verified_by_data_candidate`, `verified_by_run_candidate`) 도 phase 검증만 통과하면 episode outcomes 에 반영. **96% 빈 값 사고 (Iter 1-4) 재발 방지.**
...
- outcome 시퀀스: **휴리스틱** (turn 순회만)
- episode 합산: **결정론적 Python** (LLM 합산 금지)
...
class Episode:
    ...
    outcome_sequence: list[str]      # 시간순 outcome 등장 순서 (시퀀스 패턴 검출용)
