...
**핵심 원칙**: 집계 신호 (outcome/phase/git_intent 분포) 만 보고 패턴을 짜지 말 것. 집계 신호는 어디를 들여다볼지 가리키는 손가락일 뿐이고, 패턴 문장은 반드시 **실제 user_utterances 본문 의미**에서 도출한다. 매핑 표·체크리스트는 참고용일 뿐, 거기 없는 흐름도 본문에 있으면 패턴 후보가 된다.
...
- 사용자가 무엇을 부탁/지시하고 있는가? (집계 신호의 이름이 아니라 발화의 의미로)
...
(a) 의 후보 옆에 집계 신호로 추가 후보를 보탠다. 단, 신호는 어디를 더 들여다볼지 안내만 할 뿐, 패턴 문장은 다시 (a) 의 실제 본문에서 만든다:
...
(a) + (b) 의 후보들 묶음을 보고 **"이 그룹의 사용자는 보통 어떤 상황에서 어떤 행동 시퀀스를 하는가?"** 한 문장으로 추출. 같은 의미의 후보는 머지하고, 서로 다른 의미면 별도 패턴.

> ⚠ **약속 1 가드**: 명세의 매핑 표·체크리스트에 박힌 항목은 **참고 사례**일 뿐 검출 룰이 아니다. 거기 없는 흐름도 본문에 있으면 패턴이 되고, 거기 있어도 본문에 없으면 패턴이 아니다. **매핑 표를 채우기 위해 데이터에 없는 패턴을 만들지 말 것.**
...
```
[ ] 본문 1차 훑기 (단계 2 (a)) 에서 적어둔 후보를 다시 본다.
    데이터 신호에 들어가지 않는 흐름이라도 본문에 반복되면 후보로 유지.

[ ] outcome_distribution 의 모든 outcome 종류 한 번씩 점검
    각 outcome 이 발현된 대표 에피소드의 본문을 봐서 "어떤 작업 흐름인가" 를 본문에서 도출.

[ ] outcome_sequence (한 에피소드 안의 outcome 시간순) 의 둘러싸기 (A→B→A) / 연쇄 (A→B→C) 패턴
    의미 있는 빈도면 별도 패턴 후보. **시퀀스를 추상화로 흐리지 말고 대표 에피소드 본문의 구체성을 살릴 것.**

[ ] git_intent_distribution 세 종류 (diagnostic / output / transition) 각각 본문 확인

[ ] phase_function_groups 의 intro / main / verify 각각에서 두드러진 도구·도구 시퀀스를 본문과 같이 확인

[ ] episode_kind_distribution 세 종류 각각의 대표 본문 확인

[ ] mini_pattern_candidates 의 세 종류 (tool_microsequences / user_utterance_trigrams / tool_arg_patterns) 본문과 매칭해 의미 검증

[ ] 입력 형태별 분리 — "외부 출력을 첫 메시지로 던지기" 류는 본문 형태에 따라 별도 패턴으로 쪼개기
...
#### 사용자 사례 참고 (검출 룰 X — 본문 도출의 보조 예시)

이전 분석에서 본문 1차 훑기 단계에서 잡혔던 사용자 사례들. **이건 검출 룰이 아니다** — 이 목록에 없는 흐름도 본문에 있으면 패턴이 되고, 이 목록에 있어도 본문에 없으면 패턴이 아니다. 본인 데이터 어휘로 튜닝되지 않도록 격리해 둔다.

| 카테고리 | 본인 데이터 케이스 | 일반화한 신호 |
|---|---|---|
| 변경 후 자가 검증 시퀀스 | (1) alpha DB 로 baseline → (2) 앱·배치 직접 띄우기 → (3) curl 호출 → (4) alpha DB 로 결과 검증 | outcome 둘러싸기 시퀀스 + 대표 본문에 같은 종류 단계가 N단계 반복 |
| 종료부 — PR 코멘트 처리 | PR 코멘트 가져와 일괄 검토·반영 | 종료부 본문에 외부 피드백(코멘트/리뷰 등) 가져오기 + 반영 흐름이 함께 잡힘 |
| 외부 출력 던지기 | SQL 본문/결과 / 셸·빌드 출력 (gradlew/terraform 등) / 스택 트레이스 본문 | 사용자 첫 메시지가 일정 길이 이상의 외부 출력으로 시작 — 본문 종류로 분리 |
| 다른 직군에서는? | (예시 — 미관찰) 슬랙 스레드 본문 / Jira 티켓 본문 / 디자인 시안 URL 등 | 같은 메커니즘 — 외부 컨텍스트를 첫 메시지에 통째로 박는 흐름 |

마지막 행이 이 표의 핵심이다 — 본인 데이터 카테고리는 **다른 직군 데이터에서 전혀 다른 모양** 으로 발현될 수 있다. 명세는 카테고리를 강제하지 않고, **본문에서 어떤 종류가 반복되는지를 그 자리에서 도출**한다.
...
#### 누락 케이스 진단 절차 (실측 신호 결함 판정 시)

사용자가 짚은 패턴이 보고서에 없을 때:

```
1. turn 단위 신호가 풍부한가? (verified_by_*_candidate, git_intent, function_group)
   - 풍부함 + episode outcomes 빈약 → Stage B 합산 결함 (코드 보완 대상)
   - 풍부함 + episode outcomes 도 풍부 → Stage D 누락 (검토 체크리스트 보완)
   - 빈약함 → outcome_signals.yaml / function_groups.yaml 신호 정의 누락

2. 단일 outcome 으로 표현 불가능한 시퀀스 패턴인가?
   - outcome_sequence 점검. 사용자가 짚은 흐름과 일치하는 시퀀스가 데이터에 있는가?
   - 있으면 outcome 시퀀스 신호 추가 (위 체크리스트 참고)
   - 없으면 (3) 인지 편향 가능성

3. 위 두 경로 모두 빈약하면 → 인지 편향 결론. 보고서에 끼워맞춤 X.
```
...
**패턴 형식 (필수)**: `format_c_spec.md` "2. 패턴 한 개 형식" 의 새 포맷을 그대로 따른다.
...
핵심:
- `####` 헤더에 `A-1`, `A-2` 식 식별자 + 짧은 라벨 (2-5단어)
- `> ` 인용구로 한 줄 제목
- 본문은 1-2 문장만. 길어지면 안 됨 — 세부는 bullet 3줄로 쪼갬
- **언제 / 어떻게 / 왜 3줄 bullet 필수** — 본문 한 단락에 욱여넣지 말 것
- 사용 예시 블록은 기존 형식 (상황 + 단계별 두 줄)
- 패턴 사이 `---` 구분선
...
checklist:
  ...
  - 패턴 본문 어조가 "사용자 지시 어조" 인가? ("AI 가 ... 한다" / 모호한 "... 한다" → "... 하도록 시킨다" / "... 게 한다" / "... 게 만든다" 등으로 통일. format_c_spec.md "패턴 본문 어조" 섹션 참고)
```
...
1. **개인 MCP 서버** (`mcp_servers`) — 각 MCP 서버가 어떤 외부 시스템과 통하는지, 어떤 작업 흐름에서 호출하는지 한 줄씩. 회사 내부 mysql/grafana MCP 는 "(회사 내부)" 표기.
2. **개인 CLAUDE.md 관습** (`personal_claude_md`) — 섹션 헤더만 보고 "어떤 원칙을 자동 적용시키는지" 자연어 요약. 전문은 노출 X.
3. **개인 skill · plugin · 슬래시 커맨드** (`plugins` + `skills` + `slash_commands`) — 자주 반복되는 절차를 자동화한 묶음. 회사 내부 플러그인 (`is_company_internal: true`) 은 별도로 표기.
4. **개인 hooks · statusline · 그 외 설정** (`settings`) — hook event 별 갯수, statusline 명령, effortLevel/teammateMode 같은 토글의 의미를 자연어로.

빈 카테고리 (예: skills 0개) 는 sub-section 자체를 생략.

상세 형식·예시는 `docs/format_c_spec.md` 의 "2.5 개인 로컬 셋업 섹션" 참고.

**자가 점검**:
- [ ] 수치 나열 형태 X — 자연어 설명 톤?
- [ ] 시스템 필드 (type: undefined, scope: user 등) 본문에 노출 X?
- [ ] 회사 내부 MCP/플러그인은 "(회사 내부)" 표기로 구분됨?
- [ ] 시크릿 (토큰/키 값) 본문에 등장 0?
- [ ] 사용자 홈 경로 (`/Users/...`) 본문에 등장 0?
