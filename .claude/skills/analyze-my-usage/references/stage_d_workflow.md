...
각 패턴 본문 뒤에 코드 블록 형태의 사용 예시 추가.

**핵심**: 이 보고서는 "사용자가 어떻게 coding agent 를 프롬프트하는가" 를 잡는 문서. 사용 예시 한 단계 = **사용자가 친 메시지 한 번**.

| ❌ AI 실행 시퀀스 (틀림) | ✅ 사용자 프롬프트 시퀀스 (맞음) |
|---|---|
| `grafana mcp 로 5xx 조회` | `"5xx 떨어지는데 로그 봐줘"` |
| `git log -- src/auth.py 확인` | `"이거 언제부터 이렇게 됐지? 최근 변경 봐줘"` |
| `Edit 으로 TTL 조정` | `"이 부분 TTL 줄여줘"` |

각 단계는 따옴표로 감싼 자연어 메시지. 짧은 승인 ("응", "그대로 해줘") 도 OK — 실제로 자주 쓰는 패턴.
...
```
checklist:
  - 각 단계가 사용자 발화(따옴표로 감싼 자연어 메시지) 형태인가? AI 실행 시퀀스 X
  - 도메인 라벨이 위 4가지 중 하나인가?
  - 회사 시스템명이 들어가지 않았나? (wms/oms/logistics/kakaostyle 키워드 검색)
  - 사내 슬래시/플러그인 이름이 안 들어갔나?
  - 단계 수가 3-6 사이인가?
  - 짧은 승인("응 진행해줘") 같은 자연스러운 사용자 어조가 살아있나?
```
