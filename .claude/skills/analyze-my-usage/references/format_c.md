...
- **각 단계가 사용자가 친 메시지(자연어 프롬프트) 형태인가?** AI 실행 시퀀스 형태("grafana 로 5xx 조회") 가 아닌가?
- 도메인 라벨이 위 4가지 중 하나인가?
- 회사 시스템명이 들어가지 않았나? (wms / oms / logistics / kakaostyle 등)
- 사내 슬래시/플러그인 이름이 들어가지 않았나?
- 사용자 발화가 자연스러운가? 너무 격식 차린 명령조 X (실제 사용자는 "응 진행해줘", "그대로 해줘" 같이 친근하게 침)
- 단계 수가 3-6 사이인가?
