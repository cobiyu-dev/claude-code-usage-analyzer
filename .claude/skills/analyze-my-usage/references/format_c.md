...
도메인은 4가지 중 하나만 선택:
1. **REST API / 디버깅** — user-api, order-api, 5xx, trace_id, latency
2. **인증 / 토큰** — JWT 만료, OAuth, refresh token, session
3. **캐시 / DB** — Redis cache miss, MySQL slow query, connection pool
4. **Git / 테스트 / 빌드** — git log/bisect, npm test, gradle build, Docker

도메인 라벨 명시: `**사용 예시 (REST API 서버 디버깅):**`

회사 내부 시스템명·메서드명·사내 슬래시 스킬은 사용 예시 블록에 절대 들어가지 않음.
본문엔 OK, 예시엔 X.
