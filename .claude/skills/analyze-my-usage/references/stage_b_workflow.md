...
### (3.5-2) outcome 신호 부여 — **Stage A 가 부여한 turn 단위 신호를 합산**

**중요 변경**: yaml 매칭은 Stage A 의 `structure_labeler.py` 가 이미 수행한다. Claude(Stage B) 는 매칭 작업 자체를 하지 않고, **turn 의 `outcome_signals` 와 `git_intent` 필드를 episode 레벨로 합산만** 한다.

각 turn 의 `tool_use` 에는 다음 필드가 들어있다:
- `git_intent`: "diagnostic" | "output" | "transition" | None
- `outcome_signals`: list[str] — 이 turn 단독으로 시사하는 outcome 후보

합산 절차:

```python
def aggregate_outcomes_for_episode(ep_turns, phase_boundaries):
    ...
    for idx, turn in enumerate(ep_turns):
        for tu in turn.get("tool_uses", []):
            # git_intent 는 phase 무관 — 등장하면 부여
            if tu.get("git_intent"):
                git_intents.add(tu["git_intent"])

            # outcome 후보 처리
            for sig in tu.get("outcome_signals", []):
                # verify-phase 안인지 확인이 필요한 outcome
                in_verify = idx >= verify_start
                in_main = intro_end <= idx < verify_start

                if sig in ("committed", "pushed", "pr_opened", "pr_with_structured_body"):
                    if in_verify:
                        outcomes.add(sig)
                elif sig == "verified_by_data_candidate":
                    if in_verify:
                        outcomes.add("verified_by_data")
                elif sig == "verified_by_run_candidate":
                    if in_verify:
                        outcomes.add("verified_by_run")
                elif sig == "delegated_and_reported":
                    # main 또는 verify phase 모두 OK (마지막 turn 가까이)
                    if in_main or in_verify:
                        outcomes.add(sig)

    # incremental_commits / single_final_commit 판정
    commit_positions = []
    for idx, turn in enumerate(ep_turns):
        for tu in turn.get("tool_uses", []):
            if "committed" in tu.get("outcome_signals", []):
                phase = "intro" if idx < intro_end else ("main" if idx < verify_start else "verify")
                commit_positions.append(phase)
    ...
    # abandoned_or_paused: verify phase 비어있고 마지막 turn 에러 신호
    if verify_start >= len(ep_turns) and not outcomes:
        # 마지막 turn 의 에러 출력 또는 미응답 확인 — Stage A 의 메타에서 has_error_output 등
        if _last_turn_has_error_or_unresponded(ep_turns[-1]):
            outcomes.add("abandoned_or_paused")

    return list(outcomes), list(git_intents)
```

이 합산은 **단순 카운트·set 연산**이라 Claude 가 직접 수행하지 않아도 됨. Stage B 가 Python 으로 처리하거나, Claude 가 합산할 때 위 절차를 그대로 적용.

**약속 1 함정 회피**: 이 절차에서 임계값을 본인 데이터에 맞춰 바꾸지 말 것. 예를 들어 "incremental_commits 가 너무 자주 잡히니 임계 3 으로 올리자" 는 위반.
...
### (3.5-3) git 의도 태그 — **Stage A 가 이미 부여, Stage B 는 합산만**

각 turn 의 tool_use 에 `git_intent` 필드가 이미 있다 (Stage A 가 `git_intent_patterns.yaml` 매칭). Stage B 는 episode 안의 모든 turn 의 git_intent 를 set 으로 모은다:
