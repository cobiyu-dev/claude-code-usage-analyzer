...
같은 작업 방식 패턴을 자동으로 문서화. 도메인과 무관한 사내 공유용 보고서를 생성.
...
받는 사람은 **Claude Code 를 그냥 쓰기만 하는 한국 개발자**다. 보고서 본문은
신입 개발자가 한 번 읽고 바로 이해할 수 있는 한국어로 쓴다.

- 한자어·명사화·전문 용어 금지. 자세한 매핑은 `references/format_c.md` 의 "본문 어휘" 섹션 참조.
- 분석 도구 내부 메타 용어 (에피소드, 광역 모드, 시그니처 시퀀스 등) 가 보고서에 새지 않게 한다.
- 어조: 사내 슬랙 메시지 정도. 무게 빼고, 한 번에 읽혀야 함.
